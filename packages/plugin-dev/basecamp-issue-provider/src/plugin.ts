...
// placeholders so the plugin source carries no real credentials; users can also supply their own
// Basecamp app via the advanced clientId/clientSecret config fields below.
// NOT A SECRET — public/native-client metadata (RFC 8252), like the Google Calendar provider;
// security relies on the Launchpad redirect restriction (+ PKCE if Launchpad enforces it).
